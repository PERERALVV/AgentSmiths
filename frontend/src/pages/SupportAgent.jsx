...
const SupportAgentPage = () => {
  ...
  const markForReview = async () => {
    ...
    const filteredChat = CurrentChat.filter(chat => chat.hasOwnProperty('message') && chat.message != null && chat.message !== '');
  ...
  return (
    <SupportAgentPageContainer>
      <SupportAgentPageHeader>
        <h1>nav bar </h1>
      </SupportAgentPageHeader>
      <SupportAgentPageBody>
        <PageChatQContainer>
          <ChatList chats={Chats} onChatSelect={onChatSelect} noOngoingChat={currentChatDone}/>
        </PageChatQContainer>
        <PageActionsConytainer>
          <Actions fetch_chatlist={fetchData} markAsDone={markAsDone} review={markForReview}/>
        </PageActionsConytainer>
        <PagechatContainer>
          <Chat messages={CurrentChat} user_id={currentUser} socket={socket} setMessages={setCurrentChat} chatlength={currentchatlength}/>
        </PagechatContainer>
      </SupportAgentPageBody>
    </SupportAgentPageContainer>
  );
