...
import {
  ...
  ChatScrollDiv,
  MessageContainerDiv,
} from "../../styles/components/ChatBox";
...
function StaticChat() {
  ...
  const handleSubmit = (questionId) => {
    const allQuestionsAnswered = questionsAnswers.every(
      (q) => responses[q.id]?.answers
    );
  ...
  return (
    <ChatDiv>
      <GradientTextDiv>
        {isConnected ? "You are connected" : "Disconnected"}
      </GradientTextDiv>
      <ChatScrollDiv>
        <MessageContainerDiv>
          {questionsAnswers
            ...
            .map((questionAnswer, index) => (
              <StaticMessageContainerDiv key={index}>
                ...
                <OptionContainerDiv>
                  {questionAnswer.answer_type === "radio_button" &&
                    questionAnswer.options.map((option, idx) => (
                      <OptionDiv key={idx}>
                        <input
                          type="radio"
                          id={`radio-${index}-${idx}`}
                          ...
                          value={option}
                          onChange={(e) =>
                            handleChange(e, questionAnswer.id, "radio_button")
                          }
                          checked={
                            responses[questionAnswer.id]?.answers === option
                          }
                  ...
                  {questionAnswer.answer_type === "checkbox" &&
                    questionAnswer.options.map((option, idx) => (
                      <OptionDiv key={idx}>
                        <input
                          type="checkbox"
                          ...
                          value={option}
                          onChange={(e) =>
                            handleChange(e, questionAnswer.id, "checkbox")
                          }
                          checked={responses[
                            questionAnswer.id
                  ...
                  {questionAnswer.answer_type === "text" && (
                    <DescriptionTextArea
                      ...
                      onChange={(e) =>
                        handleChange(e, questionAnswer.id, "text")
                      }
                ...
                {!submittedQuestions.includes(questionAnswer.id) && (
                  <SubmitButton
                    type="button"
                    onClick={() => handleSubmit(questionAnswer.id)}
                  >
                    Submit
                  </SubmitButton>
                )}
          ...
          {questionsAnswers.every((q) => responses[q.id]?.answers) && (
            <StaticMessageContainerDiv>
              {responses[5]?.answers.map((answer, index) => (
                <div key={index}>
                  ...
                  <DescriptionTextArea
                    ...
                    value={
                      pageDescriptions.find((desc) => desc.pageName === answer)
                        ?.description || ""
                    }
                  />
                </div>
              ...
              <SubmitButton
                type="submit"
                onClick={handleSubmitPageDescriptions}
              >
                Submit
              </SubmitButton>
            </StaticMessageContainerDiv>
