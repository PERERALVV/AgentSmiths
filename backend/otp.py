...
from config.database import fogotPassword
...
def email_alert(subject, to):
    ...
    existing_record = fogotPassword.find_one({"email": to})
    print(existing_record)
    if existing_record:
        # Update the OTP for the existing email
        fogotPassword.update_one({"email": to}, {"$set": {"otp": otp}})
    else:
        # Insert a new record with the OTP
        fogotPassword.insert_one({"email": to, "otp": otp})
